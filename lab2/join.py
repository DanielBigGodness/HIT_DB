...
# Nested-Loop Join (NLJ)是一种基于两层循环的连接算法，它使用一张表作为外表，另一张表作为内表，对于外表的每一行，都在内表中查找匹配的行，然后输出连接结果。
...
# Sort Merge Join是一种基于排序的连接算法，它先将两张表的连接键各自进行排序，然后从两张表中分别取出一行，比较连接键的大小，
# 如果相等，就输出连接结果，如果不等，就舍弃较小的一行，继续取出下一行，重复这个过程，直到两张表中的任意一张表扫描完毕。
# 如果两张表的连接键已经有序，那么就不需要再进行排序，这时Sort Merge Join的效率会很高。Sort Merge Join适用于连接键有序，连接条件是范围的情况14。
...
# Hash Join是一种基于散列的连接算法，它使用两张表中较小的一张表（通常是驱动表）在内存中构建一个散列表，然后扫描另一张表（通常是被驱动表），
# 对每一行的连接键进行散列，然后在散列表中查找匹配的行，输出连接结果。如果散列表太大，无法一次放入内存，
# 那么会将散列表分成若干个分区，写入磁盘的临时段，这会增加I/O开销。Hash Join适用于两张表的大小差别较大，连接键没有索引，连接条件是等值的情况。
